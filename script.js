...
// const cycling = new Cycling([39, -12], 27.2, 94, 522)
...
class App {
    ...
    constructor() 
    {
      // _getPosition(), etc... gets executed when we call the 'App' class!
      // Get user's position
      this._getPosition()
    ...
    _newWorkout(event) 
    {
      ...
      if (type === 'running')
      {
          ...
          if (!validIputs(distance, duration, cadence))
      ...
      if (type === 'cycling')
      {
          ...
          // Check if data is valid
          //   if (!Number.isFinite(distance) || !Number.isFinite(duration) || !Number.isFinite(cadence))
          if (!validIputs(distance, duration, elevation))
          {
              return alert('Inputs have to be numbers')
          }
